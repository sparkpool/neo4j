...
class ErrorMessagesTest extends ExecutionEngineFunSuite with StringHelper {
  ...
  test("badMatch5") {
    expectSyntaxError(
      ...
      "Invalid input '[': expected an identifier character, whitespace, '=', node labels, a property map, a relationship pattern, ',', USING, WHERE, START, MATCH, UNWIND, MERGE, CREATE, SET, DELETE, REMOVE, FOREACH, WITH, RETURN, UNION, ';' or end of input (line 1, column 24)",
      23
    )
  }
